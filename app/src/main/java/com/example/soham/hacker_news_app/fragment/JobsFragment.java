...
public class JobsFragment extends Fragment {
                ...
                if (dy > 0) {
                    ...
                    if ((pastVisiblesItems+visibleItemCount >= totalItemCount) && (totalItemCount != 0) && (totalItemCount == (30*pages))) {
                        ...
                        for (int i=start ; i<end ; i++) {
                            try {
                                new getJobStory(getActivity(), i).execute(baseUrl +jarr.get(i).toString() + endUrl);
                            } catch (Exception e) {
                                mayScroll = false;
                            }
                        }
    ...
    class getJobStories extends AsyncTask<String, Void, JSONArray> {

        Activity context;
        JSONArray result;

        public getJobStories(){}

        public getJobStories(Activity context, JSONArray result) {
        ...
        protected void onPostExecute(JSONArray jsonArray) {
            ...
            for (int i=startPos ; i<endPos ; i++) {
                try {
                    new getJobStory(getActivity(), i).execute(baseUrl +jsonArray.get(i).toString() + endUrl);
    ...
    class getJobStory extends AsyncTask<String, Void, JSONObject> {
        ...
        public getJobStory(Activity context, int index) {
            this.context = context;
            this.index = index;
        }
